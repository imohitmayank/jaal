...
DEFAULT_OPTIONS = {
    ...
    "nodes": {"shape": "dot", "size": 7},
    "edges": {"color": {"color": "#97C2FC"}},
}
...
search_form = dbc.FormGroup(
    [
        ...
        dbc.Input(
            type="search", id="search_graph", placeholder="Search node in graph..."
        ),
        dbc.FormText(
...
def get_app_layout(graph_data, color_legends=[], directed=False, vis_opts=None):
    ...
    cat_node_features = get_categorical_features(
        pd.DataFrame(graph_data["nodes"]), 20, ["shape", "label", "id"]
    )
    try:
        cat_edge_features = get_categorical_features(
            pd.DataFrame(graph_data["edges"]).drop(columns=["color"]),
            20,
            ["color", "from", "to", "id"],
        )
    except KeyError:
        cat_edge_features = get_categorical_features(
            pd.DataFrame(graph_data["edges"]), 20, ["color", "from", "to", "id"]
        )
